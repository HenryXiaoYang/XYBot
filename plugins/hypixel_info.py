...
class hypixel_info(PluginInterface):
    ...
    def run(self, recv):
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36 QIHU 360SE'}  # 设置user agent 绕cf
    ...
    def get_in_game_name(self, soup):  # 获取玩家游戏内名字 in game name
        # ign
        in_game_name = soup.find('div', id='wrapper').find('span',
                                                           {
                                                               'style': "font-family: 'Minecraftia', serif;"}).text  # 爬虫查询，用css格式
        return in_game_name

    def get_basic_stats(self, soup):
        basic_stats = {}
        stats_bs4 = soup.find('div', id='wrapper').find_all('div', {'class': "card-box m-b-10"})[0].find_all('b')[
                    :-1]  # 爬虫查询，用css格式
        for stat in stats_bs4:  # 从爬到的数据中提取
            basic_stats[stat.get_text() + ' '] = stat.next_sibling.strip()  # 处理文本，去除空格特殊符号等
        return basic_stats

    def get_guild_stat(self, soup):
        # guild
        guild_stat = {}
        guild_bs4 = soup.find('div', id='wrapper').find_all('div', {'class': "card-box m-b-10"})[1]  # 爬虫查询，用css格式
    ...
    def get_status(self, soup):
        # status
        status = {}
        status_bs4 = soup.find('div', id='wrapper').find_all('div', {'class': "card-box m-b-10"})  # 爬虫查询，用css格式
    ...
    def get_bedwar_stat(self, soup):
        # bw
        bw_stat = []
        table = soup.find('div', id='stat_panel_BedWars').find('table', {'class': 'table'})  # 爬虫查询，用css格式
    ...
    def send_basic_info(self, recv, headers):
        ...
        req = requests.get('http://plancke.io/hypixel/player/stats/{request_ign}'.format(request_ign=request_ign),
                           headers=headers)  # 爬虫plancke，用http为了绕cf
        soup = BeautifulSoup(req.text, 'html.parser')  # 实例化爬虫处理库
    ...
    def send_bedwar_info(self, recv, headers):  # 获取玩家bedwar信息
        ...
        if req.status_code != 404 and self.check_valid(soup):  # 判断响应是否有效
