...
class manage_plugins(PluginInterface):
    ...
    async def load_plugin(self, recv):
        try:
    ...
    async def unload_plugin(self, recv):
        try:
            action_plugin = recv["content"][2]  # 获取插件名
    ...
    async def reload_plugin(self, recv):
        try:
            action_plugin = recv["content"][2]  # 获取插件名
    ...
    async def list_plugins(self, recv):
        out_message = "-----XYBot-----\n已加载插件列表："
        for plugin in plugin_manager.plugins.keys():
            out_message += f"\n{plugin}"
        logger.info(f'[发送信息]{out_message}| [发送到] {recv["from"]}')
        await self.bot.send_text_msg(recv["from"], out_message)  # 发送插件列表
