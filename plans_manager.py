...
class PlansManager:
    ...
    def load_plan(self, plan_name, myscheduler):
        if plan_name not in self.plans:
            ...
            self.plans[plan_name].run(myscheduler)

    def load_plans(self, plan_dir, myscheduler):
        for plan_file in os.listdir(plan_dir):
            if plan_file.endswith(".py") and plan_file != "__init__.py" and not plan_file.startswith('_'):
                ...
                self.load_plan(plan_name, myscheduler)
